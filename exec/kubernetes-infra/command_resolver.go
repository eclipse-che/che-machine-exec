...
type CmdResolver struct {
	...
	exec_info.KubernetesInfoExecCreator
}

// NewCmdResolver creates new instance CmdResolver.
func NewCmdResolver(shellDetector shell.ContainerShellDetector, infoExecCreator exec_info.KubernetesInfoExecCreator) *CmdResolver {
	return &CmdResolver{
		ContainerShellDetector:    shellDetector,
		KubernetesInfoExecCreator: infoExecCreator,
	}
}
...
func (cmdRslv *CmdResolver) setUpExecShellPath(exec model.MachineExec, containerInfo *model.ContainerInfo) (shellPath string, err error) {
	if containerShell, err := cmdRslv.DetectShell(containerInfo); err == nil && cmdRslv.shellIsDefined(containerShell) {
		fmt.Println("Container CMD is resolved as ", containerShell)
	...
	if err := infoExec.Start(); err != nil {
		fmt.Println("Error occurred during checking of default shell works ", err)
		return "", err
	}
	fmt.Println("Default shell is used ", shell.DefaultShell)
